...
trait TillPaymentsGateway
{
    ...
    {
        return [
            TillPaymentsPlugin::METHOD_DEBIT,
            TillPaymentsPlugin::METHOD_PREAUTHORIZE,
        ];
    }
    ...
    private function getCreditCardsPublic()
    {
        ...
        foreach ($creditCards as $cardType => $creditCard) {
            ...
            $creditCardPublic = [
                ...
                // 'viewUrl' => $this->url->link('extension/payment/till_payments_' . $this->type . '/creditCardView&cardType=' . $cardType),
            ];
            if (!empty($creditCard['seamless']) && !empty($creditCard['integration_key'])) {
